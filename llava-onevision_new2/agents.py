...
class BaseAgent:
    def __init__(self, model: LlavaOnevisionForConditionalGeneration, processor: AutoProcessor, system_prompt: str = ""):
        self.model = model
    ...
    def reset_history(self):
        """Reset conversation history, preserving system prompt."""
    ...
    def chat(self, user_content: list) -> str:
        """Multi-turn chat: appends to history."""
    ...
    def one_shot_chat(self, user_content: list) -> str:
        """One-shot inference: system + current user only, no history carry-over."""
        messages = []
...
# ==============================
# 🔹 VisualAnalyzer — V23 Q&A Chain
# ==============================
class VisualAnalyzer(BaseAgent):
    def __init__(self, model: LlavaOnevisionForConditionalGeneration, processor: AutoProcessor):
    ...
    def run_analysis_chain(self, image_object: Image.Image) -> dict:
        ...
        return {
            ...
            "has_human": has_human,
            "summary": visual_summary_text  # for compatibility
        }


# ==============================
# 🔹 CulturalAnalyzer — V23 Q&A + Category-Guided
# ==============================
class CulturalAnalyzer(BaseAgent):
    def __init__(self, model: LlavaOnevisionForConditionalGeneration, processor: AutoProcessor, country: str):
        system_prompt = (
    ...
    def run_analysis_chain(self, caption: str, visual_summary_text: str, image_object: Image.Image) -> dict:
        ...
        return {
            ...
            "summary": cultural_summary_text  # for compatibility
        }
...
class NonHateDebater(BaseAgent):
    """
    A debater arguing for the non-hateful side. 
    Its role is defined in the system prompt upon initialization.
    """
    def __init__(self, model: LlavaOnevisionForConditionalGeneration, processor: AutoProcessor):
        system_prompt = (
...
class HateDebater(BaseAgent):
    """
    A debater arguing for the hateful side.
    Its role is defined in the system prompt upon initialization.
    """
    def __init__(self, model: LlavaOnevisionForConditionalGeneration, processor: AutoProcessor):
        system_prompt = (
...
class Judge(BaseAgent):
    """
    The Judge makes a final, one-time ruling and does not need conversational memory.
    It does not use the chat history mechanism.
    """
    def __init__(self, model: LlavaOnevisionForConditionalGeneration, processor: AutoProcessor):
        self.model = model
        self.processor = processor
    ...
    def render_judgment(self, debate_transcript: str, country: str, caption: str, visual_summary_text: str, cultural_summary_text: str, image_object: Image.Image) -> str:
        ...
        messages = [{"role": "system", "content": system_prompt}, {"role": "user", "content": [{"type": "image", "image": image_object}, {"type": "text", "text": user_prompt}]}]
